...
d3.json("data/maps/election_results.json", function(data) {
	...
	function drawMaps(year) {
		...
		let yearlyData = data[year];
		let yearlyObesityRates = obesityRates[year];

		let numRedStates = 0;

		let caseValues = new Array(4);
		caseValues.fill(0);

		let mostObeseStates = [];

		for (let state in yearlyData) {
			numRedStates += yearlyData[state];
		}

		for (let state in yearlyObesityRates) {
		...
		for (let i = 0; i < numRedStates; i++) {
			mostObeseStatesObj[mostObeseStates[i][0]] = mostObeseStates[i][1];
		}

		let tooltipElection = d3.select("body").append("div")   
		    .attr("class", "tooltip-election")
		    .style("opacity", 0);

	    let tooltipObese = d3.select("body").append("div")   
		    .attr("class", "tooltip-obese")
		    .style("opacity", 0);

		let tooltipCompare = d3.select("body").append("div")   
		...
		let svgElection = d3.select('.svg-election')
				.attr("width", width)
				.attr("height", height);

		let svgObese = d3.select('.svg-obese')
				.attr("width", width)
				.attr("height", height);

		let svgCompare = d3.select('.svg-compare')
				.attr("width", width)
				.attr("height", height);
		...
		d3.json("data/maps/states_geo.json", function(json) {
			...
			for (let state in yearlyData) {
				for (let i = 0; i < json.features.length; i++)  {
					...
					let jsonState = json.features[i].properties.name;
			...
			svgElection.selectAll("path")
				...
				.style("fill", function(d) {
					let party = Number (d.properties.republican);
					return color(party);
				})
				.on("mousemove", function(d) {   
					let republican = d.properties.republican == 1 ? "Republican" : "Democrat";
		    ...
		    let election = document.querySelector('.election-heading');

			election.innerHTML = elections[year] + " - " + year;

			let legendElection = document.querySelector('.legend-election');
			...
			for (let l in legendTextElection) {
		    ...
		    let legendObese = document.querySelector('.legend-obese');
		    ...
		    svgCompare.selectAll("path")
				...
				.style("fill", function(d) {
					let party = Number (d.properties.republican);
					let obesity = d.properties.name;
					let isObese = mostObeseStatesObj.hasOwnProperty(obesity);
				...
				.on("mousemove", function(d) {   
					let party = d.properties.republican == 1 ? "Republican" : "Democrat";
		    ...
		    let legendCompare = document.querySelector('.legend-compare');
			...
			for (let l in legendTextCompare) {
			...
			let correctlyPredicted = caseValues[0] + caseValues[1];
